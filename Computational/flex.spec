...
Source: http://downloads.sourceforge.net/%{name}/flex-%{version}.tar.bz2
...
This is a gross hack to enable us to build some packages requiring
newer flex.  Please forgive us - it is only really meant to be used
in mock build environment and then thrown away.
...
( cd ${RPM_BUILD_ROOT}
  mv .%{_bindir}/flex .%{_bindir}/flex35
  mv .%{_mandir}/man1/flex.1 .%{_mandir}/man1/flex35.1
  mv .%{_libdir}/libfl.a .%{_libdir}/libfl35.a
  mv .%{_includedir}/FlexLexer.h .%{_includedir}/FlexLexer35.h
)

%find_lang flex

%post
if [ -f %{_bindir}/flex ]; then
	mv %{_bindir}/flex %{_bindir}/flex254
	ln -s %{_bindir}/flex35 %{_bindir}/flex
	mv %{_libdir}/libfl.a %{_libdir}/libfl254.a
	ln -s %{_libdir}/libfl35.a %{_libdir}/libfl.a
	mv %{_includedir}/FlexLexer.h %{_includedir}/FlexLexer254.h
	ln -s %{_includedir}/FlexLexer35.h %{_includedir}/FlexLexer.h
fi

%preun
if [ $1 = 0 ]; then
    if [ -f %{_bindir}/flex254 ]; then
	rm -f %{_bindir}/flex
	mv %{_bindir}/flex254 %{_bindir}/flex
	rm -f %{_libdir}/libfl.a
	mv %{_libdir}/libfl254.a %{_libdir}/libfl.a
	rm -f %{_includedir}/FlexLexer.h
	mv %{_includedir}/FlexLexer254.h %{_includedir}/FlexLexer.h
    fi
fi
