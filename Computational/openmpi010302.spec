...
%if "%{compiler}" == "intel110"
%define intelminrelease 083
%else
%define intelminrelease 001
%endif
%define compilershort intel
%define compilershortversion intel-%{intelmajor}
%define compilerruntimesection BuildRequires: intel-compiler%{intelmajor}-%{modulebits}-default-modules >= %{intelversion}-%{intelminrelease}\
Requires: intel-compiler%{intelmajor}-%{modulebits}-default-modules >= %{intelversion}\
Provides: openmpi-intel-runtime = %{version}-%{intelversionnum}.%{release}
%define compilerdevelsection Provides: openmpi-intel-devel = %{version}-%{intelversionnum}.%{release}
%define compilerdocsection Provides: openmpi-intel-doc = %{version}-%{intelversionnum}.%{release}
%define localdir /usr/local/intel
# for intel we need to just specify our compilers
%define compilerbuildprep export CC=icc CXX=icpc F77=ifort FC=ifort; . /etc/profile.d/modules.sh; module load intel
%define cflags -O2 -g -pipe -Wall
...
Provides: openmpi-pgi-runtime = %{version}-%{pgiversionnum}.%{release}
%define compilerdevelsection Requires: pgi-workstation >= %{pgiversion}\
Provides: openmpi-pgi-devel = %{version}-%{pgiversionnum}.%{release}
%define compilerdocsection Provides: openmpi-pgi-doc = %{version}-%{pgiversionnum}.%{release}
...
Provides: openmpi-pathscale-runtime = %{version}-%{pathscaleversionnum}.%{release}
%define compilerdevelsection Requires: pathscale-compilers >= %{pathscaleversion}\
Provides: openmpi-pathscale-devel = %{version}-%{pathscaleversionnum}.%{release}
%define compilerdocsection Provides: openmpi-pathscale-doc = %{version}-%{pathscaleversionnum}.%{release}
...
Name: %{?_name:%{_name}}%{!?_name:openmpi}010302-%{compiler}
Version: 1.3.2
Release: 18.cses.5%{?dist}
License: BSD
...
%else
BuildRequires: libibverbs-devel, opensm-devel, librdmacm-devel
%if "%{?rhel}" < "5"
...
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/share/*SPEC
...
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/ompi-server
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/orterun
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/orted
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/orte-clean
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/orte-iof
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/orte-ps
%if 0%{?suse_version} || "%{?rhel}" > "4"
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/ompi-clean
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/ompi-ps
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/ompi-checkpoint
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/ompi-iof
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/ompi-restart
%endif
# for the following four I am not sure where they belong, here or
# the -devel so for now it goes here, just in case
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/*otfaux
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/*otfconfig
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/*otfdump
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/*otfmerge
%if %{install_modulefile}
...
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/vtcc
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/vtcxx
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/vtf77
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/vtf90
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/vtfilter
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/vtunify
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/opari
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/share/vt*txt
%if 0%{?suse_version} || "%{?rhel}" > "4"
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/bin/mpi*vt
%endif
...
%dir /usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/share/vampirtrace
/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/share/vampirtrace/doc
#%doc README INSTALL LICENSE
...
#############################################################################
%changelog
* Fri May 01 2009 Josko Plazonic
