...
%endif
%define sgeloc /usr/share/sge/6.1
...
Patch100: openssh-sge.patch
BuildRequires: sge-sshdevel
Requires: openssh-server sge
License: BSD
...
# sge changes
export LDFLAGS="$LDFLAGS -L%{sgeloc}/lib/%{sgearch} -lsgeremote -luti -llck -lrmon -lpthread"
export CFLAGS="$CFLAGS -DSGESSH_INTEGRATION"
export REMOTE=sge SGE_ARCH=%{sgearch}
...
mkdir -p $RPM_BUILD_ROOT/%{sgeloc}/bin/%{sgearch}
install -m755 sshd $RPM_BUILD_ROOT/%{sgeloc}/bin/%{sgearch}/

cat > $RPM_BUILD_ROOT/%{sgeloc}/bin/%{sgearch}/qlogin_wrapper <<ENDQLOGINWRAPPER
#!/bin/sh
HOST=\$1
PORT=\$2
/usr/bin/ssh -t -X -p \$PORT \$HOST
ENDQLOGINWRAPPER
...
%pre
/usr/sbin/semanage fcontext -a -t textrel_shlib_t %{sgeloc}/bin/%{sgearch}/sshd >/dev/null 2>&1 || :

%post
/sbin/fixfiles restore %{sgeloc}/bin/%{sgearch}/sshd >/dev/null 2>&1 || :

%preun
if [ "$1" = "0" ]; then
        /usr/sbin/semanage fcontext -d -t textrel_shlib_t %{sgeloc}/bin/%{sgearch}/sshd >/dev/null 2>&1 || :
fi

%files
%defattr(-,root,root)
%doc LICENCE 
%attr(0755,root,root) %{sgeloc}/bin/%{sgearch}/sshd
%attr(0755,root,root) %{sgeloc}/bin/%{sgearch}/qlogin_wrapper

%changelog
* Tue Sep 25 2007 Josko Plazonic <plazonic@math.princeton.edu>
- convert to use with sge - i.e. compile against sge and add
  also the qlogin_wrapper
...
* Fri Nov 10 2006 Tomas Mraz <tmraz@redhat.com> - 4.3p2-10.1
