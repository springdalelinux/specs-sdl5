...
Patch100: openssh-5.3p1-hpn13v7-puias.diff
...
Group: Applications/Internet
Requires: openssh-hpn = %{version}-%{release}
Obsoletes: openssh-hpn-askpass-gnome
Provides: openssh-hpn-askpass-gnome
...
%configure \
	--sysconfdir=%{_sysconfdir}/ssh-hpn \
	--libexecdir=%{_libexecdir}/openssh-hpn \
	--datadir=%{_datadir}/openssh-hpn \
	--with-tcp-wrappers \
	--with-default-path=/usr/local/bin:/bin:/usr/bin \
	--with-superuser-path=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin \
	--with-privsep-path=%{_var}/empty/sshd \
	--enable-vendor-patchlevel="PU_IAS-%{version}-%{release}-hpn" \
	--disable-strip \
...
# we will use -hpn versions in our executables:
perl -pi -e 's|SSH_PROGRAM=/usr/bin/ssh|SSH_PROGRAM=/usr/bin/ssh-hpn|' Makefile
perl -pi -e 's|/usr/bin/ssh|/usr/bin/ssh-hpn|' pathnames.h
perl -pi -e 's|sshd.pid|sshd-hpn.pid|' Makefile pathnames.h
...
%define __spec_install_post \
    ...
    %{__arch_install_post} \
    %{__os_install_post} \
    fipshmac $RPM_BUILD_ROOT%{_bindir}/ssh-hpn \
    fipshmac $RPM_BUILD_ROOT%{_sbindir}/sshd \
%{nil}

%install
rm -rf $RPM_BUILD_ROOT
mkdir -p -m755 $RPM_BUILD_ROOT%{_sysconfdir}/ssh-hpn
mkdir -p -m755 $RPM_BUILD_ROOT%{_libexecdir}/openssh-hpn
mkdir -p -m755 $RPM_BUILD_ROOT%{_var}/empty/sshd
make install DESTDIR=$RPM_BUILD_ROOT

install -d $RPM_BUILD_ROOT/etc/pam.d/
install -d $RPM_BUILD_ROOT/etc/rc.d/init.d
install -d $RPM_BUILD_ROOT%{_libexecdir}/openssh-hpn
install -m644 %{SOURCE2} $RPM_BUILD_ROOT/etc/pam.d/sshd
install -m755 %{SOURCE3} $RPM_BUILD_ROOT/etc/rc.d/init.d/sshd
install -m755 contrib/ssh-copy-id $RPM_BUILD_ROOT%{_bindir}/ssh-copy-id-hpn
perl -pi -e 's|ssh-add|ssh-add-hpn|' $RPM_BUILD_ROOT%{_bindir}/ssh-copy-id-hpn
install contrib/ssh-copy-id.1 $RPM_BUILD_ROOT%{_mandir}/man1/

%if ! %{no_gnome_askpass}
install -s contrib/gnome-ssh-askpass $RPM_BUILD_ROOT%{_libexecdir}/openssh-hpn/gnome-ssh-askpass
%endif

%if ! %{scard}
	rm -f $RPM_BUILD_ROOT%{_datadir}/openssh-hpn/Ssh.bin
%endif

%if ! %{no_gnome_askpass}
ln -s gnome-ssh-askpass $RPM_BUILD_ROOT%{_libexecdir}/openssh-hpn/ssh-askpass
...
mv $RPM_BUILD_ROOT/%{_bindir}/scp $RPM_BUILD_ROOT/%{_bindir}/scp-hpn
mv $RPM_BUILD_ROOT/%{_bindir}/sftp $RPM_BUILD_ROOT/%{_bindir}/sftp-hpn
mv $RPM_BUILD_ROOT/%{_bindir}/ssh $RPM_BUILD_ROOT/%{_bindir}/ssh-hpn
pushd $RPM_BUILD_ROOT/%{_bindir}
rm -f slogin
ln -s ssh-hpn slogin-hpn
popd
mv $RPM_BUILD_ROOT/%{_bindir}/ssh-add $RPM_BUILD_ROOT/%{_bindir}/ssh-add-hpn
mv $RPM_BUILD_ROOT/%{_bindir}/ssh-agent $RPM_BUILD_ROOT/%{_bindir}/ssh-agent-hpn
mv $RPM_BUILD_ROOT/%{_bindir}/ssh-keygen $RPM_BUILD_ROOT/%{_bindir}/ssh-keygen-hpn
mv $RPM_BUILD_ROOT/%{_bindir}/ssh-keyscan $RPM_BUILD_ROOT/%{_bindir}/ssh-keyscan-hpn
...
%attr(0755,root,root) %dir %{_sysconfdir}/ssh-hpn
%attr(0600,root,root) %config(noreplace) %{_sysconfdir}/ssh-hpn/moduli
%if ! %{rescue}
%attr(0755,root,root) %{_bindir}/ssh-keygen-hpn
%attr(0644,root,root) %{_mandir}/man1/ssh-keygen-hpn.1*
%attr(0755,root,root) %dir %{_libexecdir}/openssh-hpn
%attr(4755,root,root) %{_libexecdir}/openssh-hpn/ssh-keysign
%attr(0644,root,root) %{_mandir}/man8/ssh-keysign-hpn.8*
%endif
%if %{scard}
%attr(0755,root,root) %dir %{_datadir}/openssh-hpn
%attr(0644,root,root) %{_datadir}/openssh-hpn/Ssh.bin
%endif

%files clients
%defattr(-,root,root)
%attr(0755,root,root) %{_bindir}/ssh-hpn
%attr(0644,root,root) %{_bindir}/.ssh-hpn.hmac
%attr(0644,root,root) %{_mandir}/man1/ssh-hpn.1*
%attr(0755,root,root) %{_bindir}/scp-hpn
%attr(0644,root,root) %{_mandir}/man1/scp-hpn.1*
%attr(0644,root,root) %config(noreplace) %{_sysconfdir}/ssh-hpn/ssh_config
%attr(0755,root,root) %{_bindir}/slogin-hpn
%attr(0644,root,root) %{_mandir}/man1/slogin-hpn.1*
%attr(0644,root,root) %{_mandir}/man5/ssh_config-hpn.5*
%if ! %{rescue}
%attr(2755,root,nobody) %{_bindir}/ssh-agent-hpn
%attr(0755,root,root) %{_bindir}/ssh-add-hpn
%attr(0755,root,root) %{_bindir}/ssh-keyscan-hpn
%attr(0755,root,root) %{_bindir}/sftp-hpn
%attr(0755,root,root) %{_bindir}/ssh-copy-id-hpn
%attr(0644,root,root) %{_mandir}/man1/ssh-agent-hpn.1*
%attr(0644,root,root) %{_mandir}/man1/ssh-add-hpn.1*
%attr(0644,root,root) %{_mandir}/man1/ssh-keyscan-hpn.1*
%attr(0644,root,root) %{_mandir}/man1/sftp-hpn.1*
%attr(0644,root,root) %{_mandir}/man1/ssh-copy-id-hpn.1*
%endif
...
%attr(0755,root,root) %{_libexecdir}/openssh-hpn/sftp-server
%attr(0644,root,root) %{_mandir}/man5/sshd_config-hpn.5*
%attr(0644,root,root) %{_mandir}/man5/moduli-hpn.5*
%attr(0644,root,root) %{_mandir}/man8/sshd-hpn.8*
%attr(0644,root,root) %{_mandir}/man8/sftp-server-hpn.8*
%attr(0600,root,root) %config(noreplace) %{_sysconfdir}/ssh-hpn/sshd_config
...
%attr(0755,root,root) %{_libexecdir}/openssh-hpn/gnome-ssh-askpass
%attr(0755,root,root) %{_libexecdir}/openssh-hpn/ssh-askpass
%endif
