...
#
# Intel 10.0 compiler
%if "%{compiler}" == "intel100"
%define compilershort intel
%define compilerruntimesection BuildRequires: intel-compiler100-%{modulebits}-default-modules\
Requires: intel-compiler100-%{modulebits}-default-modules\
Provides: openmpi-intel-runtime = %{version}-100.%{release}
%define compilerdevelsection Provides: openmpi-intel-devel = %{version}-100.%{release}
%define compilerdocsection Provides: openmpi-intel-doc = %{version}-100.%{release}
%define localdir /usr/local/intel
# for intel we need to just specify our compilers
%define compilerbuildprep export CC=icc CXX=icpc F77=ifort FC=ifort; . /etc/profile.d/modules.sh; module load intel
%define cflags -O2 -g -pipe -Wall
...
%define compilerldflags %{nil}
# this is for configuring modules
%define compilermodulename intel-10.0
%define compilerloadmodule module load intel/10.0/%{modulebits}
%define compilerconflictmodule mpich/ mpich2/ lam/ pgi/
%define compilermodulefile .modulerc-intel-%{modulebits}-100
%endif
#
# PGI compiler
%if "%{compiler}" == "pgi70"
%define compilershort pgi
%define compilerruntimesection BuildRequires: pgi-workstation70\
Requires: pgi-workstation70-libs\
Provides: openmpi-pgi-runtime = %{version}-070.%{release}
%define compilerdevelsection Requires: pgi-workstation70\
Provides: openmpi-pgi-devel = %{version}-070.%{release}
%define compilerdocsection Provides: openmpi-pgi-doc = %{version}-070.%{release}
...
%define compilerldflags %{nil}
# this is for configuring modules
%define compilermodulename pgi-7.0
%define compilerloadmodule module load pgi/7.0/%{modulebits}
%define compilerconflictmodule mpich/ mpich2/ lam/ intel/
%define compilermodulefile .modulerc-pgi-%{modulebits}-070
%endif
...
Name: %{?_name:%{_name}}%{!?_name:openmpi}010204-%{compiler}
Version: 1.2.4
Release: 2.cses.5%{?dist}
License: BSD
...
BuildRequires: libtool, numactl-devel
Requires: libtool, numactl-devel
...
%configure %{cluster_options} %{_with_openib} %{configure_options} %{pvfs2_build_flags} \
		...
		--with-wrapper-ldflags="-L%{localdir}/%{_lib} -L%{localdir}/%{_lib}/openmpi %{pvfs2_ld_flags}" \
		LDFLAGS="-Wl,-rpath,/usr/local/openmpi/%{version}/%{compiler}/%{_target_cpu}/lib %{pvfs2_ld_flags} %{compilerldflags}"
...
mkdir -p %dir $RPM_BUILD_ROOT/%{localdir}/%{_lib}/openmpi
...
prepend-path LD_LIBRARY_PATH "%{_libdir}:%{localdir}/%{_lib}/openmpi"
prepend-path MANPATH "%{_mandir}"
append-path -delim { } LOCAL_LDFLAGS "-L%{_libdir} -L%{localdir}/%{_lib}/openmpi"
EOF
%endif
