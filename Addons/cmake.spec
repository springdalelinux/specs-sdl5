...
# Which we do by default on RHEL5, because: The installed Qt version
# 4.2.1 is too old, at least version 4.3.0 is required
%bcond_with gui
...
%if %{with gui}
# We have this dependency anyway, via QT, and we put stuff into
# /usr/share/mime, so let's make the dependency explicit.
...
configuration, preprocessor generation, code generation, and template
instantiation.
...
install -p -m0644 Copyright.txt $RPM_BUILD_ROOT%{_datadir}/doc/%{name}-%{version}/
...
%changelog
* Mon Feb 28 2011 Petr Machata <pmachata@redhat.com> - 2.6.4-5.el5.4
- Add requires: shared-mime-info when building with gui
...
* Thu Feb 10 2011 Petr Machata <pmachata@redhat.com> - 2.6.4-5.el5.3
- Disable building with GUI, RHEL 5 doesn't have recent-enough qt4
